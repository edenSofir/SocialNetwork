class MsgIndicator extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.state.interval_id = setInterval(() => { this.check_for_new_messages() }, 3000);
    }

    async check_for_new_messages() {
        let messages_to_compare = await this.fetch_messages();
        if (messages_to_compare.length > this.state.amout_of_messages && !this.state.new_messages) {
            this.state.new_messages = true;
            this.setState({ amout_mesages_diffrent: messages_to_compare.length - this.state.amout_of_messages, amout_of_messages: messages_to_compare.length});
        }
    }
    ...
    render() {
        return <div className="amount_new_posts">
            <button type="button" value='message' onClick={this.indicator_message}>amount of new messages:{this.state.amout_mesages_diffrent}</button>
        </div>
    }
}
...
class PostIndicator extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.state.interval_id = setInterval(() => { this.check_for_new_posts() }, 3000);
    }

    async check_for_new_posts() {
        let posts_to_compare = await this.fetch_posts();
        if (posts_to_compare.length > this.state.amout_of_posts && !this.state.new_posts){
    ...
    render() {
        return <div className="amount_new_posts">
        <button type="button" value='post' onClick={this.indicator_post}> amount of new posts: {this.state.amout_posts_diffrent} </button>
        </div>
    }
}